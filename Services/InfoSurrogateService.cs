...
namespace Surrogacy.Service
{
    public class InfoSurrogateService
    {
        public InfoSurrogate ViewInfoSurrogate(InfoSurrogate infosurrogate, int SurrogateID)
        {
            ...
            var UserID = ApplicationManager.LoggedInUser.UserID;
            DataSet dataSet = new DataSet();
            try
            {
                dataSet = infosurrogatedata.ViewInfoSurrogate(infosurrogate,SurrogateID,UserID);
                ...
                if (dataSet.Tables["InfoSurrogate"].Rows.Count > 0)
                {
                    ...
                    localinfosurrogate.Description = dataSet.Tables["InfoSurrogate"].Rows[0]["DESCRIPTION"].ToString();
                }
            }
